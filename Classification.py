...
@st.cache_data
def showSaliencyMap(_img, _model, _pred_position):
    images = tf.Variable(_img, dtype=float)
    with tf.GradientTape() as tape:
        pred = _model(images, training=False)
        with _pred_position:
            st.write("")
            st.write("")
            st.write("Prediction:")
...
@st.cache_resource
def loadmodel():
    return tf.keras.models.load_model('ResNet50_test.h5')
